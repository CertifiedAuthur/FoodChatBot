...
def place_new_order(parameters: dict, session_id: str):
    ...
    if session_id not in inprogress_orders:
        return JSONResponse(content={
            "fulfillment_text": "Starting new order. Specify food items and quantities. For example, you can say,  \
            'I would like to order two pizzas and one mango lassi. Also, we have only the following items on our menu: \
...
def remove_from_order(parameters: dict, session_id: str):
    ...
    if len(no_such_items) > 0:
        fulfillment_text = f'Your current order does not have {",".join(no_such_items)} from your order!'
